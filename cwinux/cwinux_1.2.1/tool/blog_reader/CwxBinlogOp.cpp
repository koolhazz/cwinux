...
int CwxBinlogOp::init(string const& strBinLogFileName){
    ...
    if (0 != m_pCursor->open(strBinLogFileName.c_str())){
...
int CwxBinlogOp::doCommand(char* szCmd){
    ...
    }else if(0 == strcasecmp((*iter).c_str(), "type")){
        if (2 != uiItemNum){
            printf("Invalid type command, using: type value.\n");
            return 1;
        }
        iter++;
        doType(strtoul((*iter).c_str(), NULL, 0));
...
void CwxBinlogOp::doType(CWX_UINT32 uiType){
    int iRet = 0;
    printf("Finding binlog with type %u\n", uiType);
    while(1){
        if (m_pCursor->getHeader().getType() == uiType){
            return ;
        }
        iRet = m_pCursor->next();
        if (0 == iRet){
            printf("Can't find binlog with type %u\n", uiType);
            return ;
        }else if (0 > iRet){
            printf("Failure to iterator binlog file, error: %s\n", m_pCursor->getErrMsg());
            return ;
